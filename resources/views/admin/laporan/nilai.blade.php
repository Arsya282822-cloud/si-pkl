@extends('layouts.app')
@section('title', 'Rekapitulasi Nilai PKL')
@section('topbar_title', 'Rekapitulasi Nilai PKL')
...
<div class="d-flex justify-content-between align-items-center mb-4 no-print">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('admin.export.nilai') }}" class="btn btn-success">
...
<div class="card dashboard-card">
    <div class="card-body p-4">
        ...
        <form class="row g-2 mb-4 no-print" method="GET">
            <div class="col-md-3">
                <input name="q" value="{{ $q }}" class="form-control" placeholder="Cari nama siswa / NIS..." style="border-radius: 8px;">
            </div>
            <div class="col-md-3">
            ...
            @if($q || $periodeId || $jurusanId)
                <div class="col-auto">
                    <a href="{{ route('admin.laporan.nilai') }}" class="btn btn-outline-secondary" style="border-radius: 8px;">Reset</a>
                </div>
            @endif
        </form>
            ...
            <table class="table align-middle table-bordered table-hover">
                <thead class="table-light text-center">
                    <tr>
                        ...
                        <th class="text-start">Perusahaan & Pembimbing</th>
                        <th>Sikap</th>
                        <th>Keterampilan</th>
                        <th>Pengetahuan</th>
                        <th>Nilai Akhir</th>
                ...
                <tbody>
                    @forelse($penempatanList as $i => $item)
                        @php
                            ...
                            if ($nilaiAkhir !== null) {
                                if ($nilaiAkhir >= 90) { $predikat = 'A (Sangat Baik)'; $badgeClass = 'bg-success'; }
                                elseif ($nilaiAkhir >= 80) { $predikat = 'B (Baik)'; $badgeClass = 'bg-primary'; }
                                elseif ($nilaiAkhir >= 70) { $predikat = 'C (Cukup)'; $badgeClass = 'bg-warning text-dark'; }
                                else { $predikat = 'D (Kurang)'; $badgeClass = 'bg-danger'; }
                            }
                        @endphp
                        <tr>
                            ...
                            <td>
                                <div class="fw-semibold">{{ $item->perusahaan?->nama_perusahaan ?? '-' }}</div>
                                <small class="text-muted">Guru: {{ $item->guru?->nama ?? '-' }}</small>
                            </td>
                            <td class="text-center">{{ $nilai?->nilai_sikap ?? '-' }}</td>
                            <td class="text-center">{{ $nilai?->nilai_keterampilan ?? '-' }}</td>
                            <td class="text-center">{{ $nilai?->nilai_pengetahuan ?? '-' }}</td>
                            <td class="text-center fw-bold fs-6 {{ $nilaiAkhir ? 'text-primary' : 'text-muted' }}">
                                {{ $nilaiAkhir !== null ? number_format($nilaiAkhir, 1) : 'Belum Dinilai' }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="10" class="text-center text-muted py-5">Belum ada data nilai untuk filter ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
