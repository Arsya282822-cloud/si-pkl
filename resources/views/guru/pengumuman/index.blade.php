@extends('layouts.app')
@section('title', 'Pesan & Pengumuman Resmi')
@section('content')
...
<div class="pro-card p-3 mb-4" style="border-radius: 14px;">
    <form method="GET" action="{{ route('guru.pengumuman.index') }}" class="row g-2 align-items-center">
        <div class="col-md-5">
            <div class="input-group input-group-sm">
                <span class="input-group-text bg-white border-end-0"><i class="ph ph-magnifying-glass text-muted"></i></span>
                <input type="text" name="q" class="form-control border-start-0" placeholder="Cari judul atau isi pengumuman..." value="{{ $q }}">
            </div>
        </div>
        <div class="col-md-4">
            <select name="kategori" class="form-select form-select-sm" onchange="this.form.submit()">
                ...
                <option value="peringatan" {{ $kategori == 'peringatan' ? 'selected' : '' }}>Peringatan / Evaluasi</option>
            </select>
        </div>
        <div class="col-md-3 d-flex gap-2">
            ...
            @if($q || $kategori)
                <a href="{{ route('guru.pengumuman.index') }}" class="btn btn-sm btn-light border text-muted">Reset</a>
            @endif
        </div>
    </form>
</div>
            ...
            <div class="pro-card p-4 border-0 shadow-sm" style="background-color: {{ $alertBg }}; border-left: 6px solid {{ $borderCol }} !important; border-radius: 14px;">
                ...
                @if($p->file_lampiran)
                    <div class="mt-3 pt-3 border-top d-flex align-items-center justify-content-between" style="border-color: rgba(0,0,0,0.08) !important;">
                        <small class="text-muted"><i class="ph ph-paperclip me-1"></i>Lampiran Berkas Resmi</small>
                        <a href="{{ asset($p->file_lampiran) }}" target="_blank" class="btn btn-sm btn-white border shadow-sm text-primary fw-semibold d-inline-flex align-items-center gap-1.5" style="border-radius: 8px; font-size: 0.825rem;">
                            <i class="ph ph-file-arrow-down" style="font-size: 16px;"></i> Unduh Berkas Lampiran
                        </a>
                    </div>
                @endif
            </div>
    ...
    @empty
        <div class="col-12">
            <div class="pro-card p-5 text-center text-muted" style="border-radius: 16px;">
                ...
                <p class="small text-muted mb-0">Belum ada pengumuman atau broadcast pesan untuk kategori ini.</p>
            </div>
        </div>
    @endforelse
</div>
